...
Eine Exposition ist eine Kubernetes-Custom-Resource, die definiert, wie ein Service von außerhalb des Clusters erreichbar gemacht wird.
Sie unterstützt HTTP-Routen (Layer 7) sowie rohe TCP- und UDP-Ports (Layer 4).
Für jede Exposition erstellt der [Operator](https://github.com/cloudogu/k8s-service-discovery) die notwendigen Kubernetes-nativen Objekte und weist für TCP/UDP-Einträge einen Port am LoadBalancer-Service des Clusters zu.
...
spec:
  http:
    - name: ui
      service: myapp
      port: 8080
      # Unter dieser URL ist die App von außen erreichbar, https://<fqdn>/myapp/dashboard
      path: /myapp
      rewrite:
        # Entfernt /myapp aus der URL – wird intern als /dashboard an den Service (myapp:8080) weitergeleitet.
        stripPrefix: /myapp
```
